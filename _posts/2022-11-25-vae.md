...
where $h^{(1)}\_\phi$ and $h^{(1)}\_\phi$ are two neural networks that map the original object, $\boldsymbol{x}$, to the mean, $\boldsymbol{\mu}$, and the logarithm of the variance, $\log \boldsymbol{\sigma}^2$, of the approximate posterior respectively. $R$ is the number of parameters to these neural networks. Note that $h^{(1)}\_\phi$ and $h^{(2)}\_\phi$ may either be two entirely separate neural networks or may share some subset of parameters as shown below:
...
We will use to $h_\phi$ to refer to the full neural network encompassing both $h^{(1)}\_\phi$ and $h^{(2)}\_\phi$.
