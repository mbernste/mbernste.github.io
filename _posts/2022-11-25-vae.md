...
The name "variational autoencoder" implies that these models are a type of [autoencoder](https://en.wikipedia.org/wiki/Autoencoder), in the usual machine learning sense, as a model that takes a data point, $\boldsymbol{x}$, compresses it into a low-dimensional data point $\boldsymbol{z}$, and then reconstructs, as best as it can, back into $\boldsymbol{x}$. As a review, autoencoders can be visualized as follows:
...
Here we see one function (usually a neural network) compress $\boldsymbol{x}$ into a low-dimensional data point, $\boldsymbol{z}$, and then another function (also a neural network) decompresses it back into an approximation of $\boldsymbol{x}$, here denoted as $\boldsymbol{x}'$.  

As their name suggests VAEs can indeed be viewed as a type of autoencoder; however, their theoretical foundations are rooted in a quite different field: Bayesian inference. As we opened up this blog post, VAEs are truly probablistic models. As we will see in this post, their "autoencoding" nature arises indirectly from the way in which we setup the [variational inference](https://mbernste.github.io/posts/variational_inference/)-based procedure for fitting these models.

In summary, it helps to [view VAEs from multiple angles](https://mbernste.github.io/posts/understanding_3d/) (as is the case with most complicated ideas). Those are two angles are:
...
In this blog post we will first present VAEs through both of these lenses. We will then present an example of running VAEs on MNIST. Finally, we will discuss an application of VAEs in computational biology (the original reason I became interested in these models!). Specfically, we will discuss the [scVI method](https://docs.scvi-tools.org/en/stable/user_guide/models/scvi.html) by [Lopez et al. (2018)](https://www.nature.com/articles/s41592-018-0229-2), which is a VAE-based model used for analyzing [single-cell RNA-seq](https://en.wikipedia.org/wiki/Single_cell_sequencing) data. This case-study will provide an example of a real-world use-case for VAEs.
