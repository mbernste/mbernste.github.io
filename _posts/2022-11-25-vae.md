...
where $h^{(1)}\_\phi$ and $h^{(1)}\_\phi$ are two neural networks that map the original object, $\boldsymbol{x}$, to the mean, $\boldsymbol{\mu}$, and the logarithm of the variance, $\log \boldsymbol{\sigma}^2$, of the approximate posterior respectively. Note that $h^{(1)}\_\phi$  and $h^{(2)}\_\phi$ may either be two entirely separate neural networks, or may share common paramters as shown below:

&nbsp;

<center><img src="https://raw.githubusercontent.com/mbernste/mbernste.github.io/master/images/VAE_encoder_architecture.png" alt="drawing" width="700"/></center>

&nbsp;

We will refer to $h_\phi$ as the full neural network encompassing both $h^{(1)}\_\phi$ and $h^{(2)}\_\phi$.


Further, $R$ is the number of parameters to these neural networks. Said a second way, we define $q\_\phi(\boldsymbol{z} \mid \boldsymbol{x})$ as
