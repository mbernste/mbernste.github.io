...
Aside from those two differences, the two implementations are quite similar.  Below, we show code implementing a simple VAE. Note that here we sample onle one value of $\epsilon_{i}$ per data point (that is, $L := 1$). In practice, if the training set is large enough, a single sample per datapoint often suffices to achieve good performance. Key differences in the code between the standard autoencoder and VAE are highlighted in red:
...
class VAE(nn.Module):
    def __init__(
        ...
        # Define autoencoding layers
        self.enc_layer1 = nn.Linear(x_dim, hidden_dim)
        <span style="color:red">some *blue* text</span>.self.enc_layer2_mu = nn.Linear(hidden_dim, z_dim)</span> 
        <span style="color:red">some *blue* text</span>.self.enc_layer2_logvar = nn.Linear(hidden_dim, z_dim)</span> 
