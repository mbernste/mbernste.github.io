...
3\. **Modeling complex distributions:** VAEs can be extended to model more complex distributions of the data $\boldsymbol{x}_1, \dots, \boldsymbol{x}_n$. That is, VAEs provide a principled way to learn low dimensional representations of for datasets that are distributed according to more complicated distributions. One example of an application of VAEs using non-Gaussian distributions comes from [single-cell RNA-seq](https://en.wikipedia.org/wiki/Single-cell_transcriptomics) analysis in the field of genomics.  [scVI](https://scvi-tools.org/) is a tool that uses a VAE to model vectors of counts that are assumed to be distributed according to a [zero-inflated](https://en.wikipedia.org/wiki/Zero-inflated_model) negative binomial distribution. Specifically, its probabilistic model as described by [Lopez et al. (2018)](https://www.nature.com/articles/s41592-018-0229-2) is given by:

$$\begin{align*} \boldsymbol{z}_i &\sim N(\boldsymbol{0}, \boldsymbol{I}) \\ l_i &\sim \text{LogNormal}(l_{\mu}, l_{\sigma}^2) \\ \rho_i &\sim f_{w}(z_n, s_n) \\ &= \boldsymbol{y}_i &\sim \text{Gamma}(\boldsymbol{\rho}_i, \Psi) \\ \boldsymbol{h}_i \sim \text{Bernoulli}(f_h(\boldsymbol{z}_i, s_i)) \\ &= \boldsymbol{x}_i := \boldsymbol{h}_i \bigodot \boldsymbol{y}_i \end{align*}$$

where $\bigodot$ denotes element-wise multiplication. $f_{w}$ and $f_{h}$ are both neural networks where $w$ and $h$ are the parameters to those neural networks. The full model parameters here are $\theta := \{\omega, w, h, \dots, s_i, \Psi\}$. The hyperparameters are $l_{\mu}$ and $l_{\sigma}^2$.

The variational posterior has the same form as we have considered so far in this blog post:
