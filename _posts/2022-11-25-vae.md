...
Appendix: Derivation of KL-divergence
-------------------------------------

If, for our VAE, we choose the variational distribution $q_\phi(\boldsymbol{z}_i \mid \boldsymbol{x}_i)$ is defined to be a normal distribution:

$$q_\phi(\boldsymbol{z}_i \mid \boldsymbol{x}_i) := N(h^{(1)}_\phi(\boldsymbol{x}_i), \text{diag}(\exp(h^{(2)}_\phi(\boldsymbol{x}_i))))$$

and the prior over over $\boldsymbol{z}_i$ to be a standard normal distribution:

$$p(\boldsymbol{z}) := N(\boldsymbol{0}, \boldsymbol{I})$$

then the KL-divergence from $p(\boldsymbol{z})$ to $q_\phi(\boldsymbol{z}_i \mid \boldsymbol{x}_i)$ is given by the following formula:

$$KL(q_\phi(\boldsymbol{z}_i \mid \boldsymbol{x}_i) \mid\mid p(\boldsymbol{z})) = -\frac{1}{2} \sum_{j=1}^J \left(1 + h^{(2)}_\phi(\boldsymbol{x}_i)_j - h^{(1)}_\phi(\boldsymbol{x}_i)_j^2 - \exp(h^{2}_\phi(\boldsymbol{x}_i)_j) \right)$$

**Proof:**

For ease of notation, we will drop the index $i$ from $\boldsymbol{x}_i$ and $\boldsymbol{z}_i$, given that we will only consider a single data point/observation here. Further, let 

$$\begin{align*}\mu &:= h^{(1)}_\phi(\boldsymbol{x}) \\ \sigma^2 &:= \exp(h^{(2)}_\phi(\boldsymbol{x})) \end{align*}$$

be the mean posterior mean and variance for $\boldsymbol{x}$.
...
First, let us compute $\int q_\phi(\boldsymbol{z} \mid \boldsymbol{x}) \log q_\phi(\boldsymbol{z} \mid \boldsymbol{x}) \ d\boldsymbol{z}$:

$$\begin{align*}\int q_\phi(\boldsymbol{z} \mid \boldsymbol{x}) \log q_\phi(\boldsymbol{z} \mid \boldsymbol{x}) \ d\boldsymbol{z} &= \int N(\boldsymbol{z}; \boldsymbol{\mu}, \text{diag}(\boldsymbol{\sigma}^2)) \log N(\boldsymbol{z}; \boldsymbol{0}, \boldsymbol{I}))\end{align*}$$
