...
It turns out that, due to Mercer's Theorem, for input vectors $\boldsymbol{x} \in \mathbb{R}^n$ (i.e., where the space $\mathcal{X}$ are coordinate vectors), there is another way to represent $\phi(x)$ that is more in line with the idea of $\phi(x)$ mapping $x$ to a new set of "features". Before we arrive at this new representation, let's state Mercer's Theorem:

<span style="color:#0060C6">**Theorem 3 (Mercer's Theorem):** Let $K$ be a symmetric, positive-definite function defined over . Then, define the linear operator</span>

<center><span style="color:#0060C6">$$T_{K} : L^2 \rightarrow \mathbb{R}$$ $$</span></center>

<span style="color:#0060C6">such that for a given function $\phi \in L^2$,</span>

<center><span style="color:#0060C6">$$T_{K}(\phi)(\boldsymbol{x}) := \int_{a, b} K(\boldsymbol{x}, \boldsymbol{s}) \phi(\boldsymbol{s}) d\boldsymbol{s}$$
...
<center><span style="color:#0060C6">$$K(\boldsymbol{x}, \boldsymbol{y}) = \sum_{i=1}^\infty \lambda_i \psi(\boldsymbol{x} \psi(\boldsymbol{y})$$</span></center>

The statement in this theorem is rather complex
