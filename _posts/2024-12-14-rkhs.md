...
<span style="color:#0060C6">**Theorem 3 (Mercer's Theorem):** Let $K$ be a symmetric, positive-definite function defined over a [compact set](https://en.wikipedia.org/wiki/Compact_space) S \subset \mathbb{R}^n. Let $(S, \Sigma, \mu)$ be a [measure space](https://mbernste.github.io/posts/measure_theory_1/) defined over $S$ and $L^2(S, \mu)$ be the space of $L^2$ functions. Then, define the linear operator</span>

<center><span style="color:#0060C6">$$T_{K} : L^2_\mu \rightarrow \mathbb{R}$$ $$</span></center>

<span style="color:#0060C6"> where $\mu$ is a measure function, such that for a given function $\phi \in L^2$,</span>

<center><span style="color:#0060C6">$$T_{K}(\phi)(\boldsymbol{x}) := \int_{a, b} K(\boldsymbol{x}, \boldsymbol{s}) \phi(\boldsymbol{s}) d\boldsymbol{s}$$</span></center>
