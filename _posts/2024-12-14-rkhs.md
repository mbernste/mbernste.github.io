...
It turns out that, due to Mercer's Theorem, there is another way to represent $\phi(x)$ that is more in line with the idea of $\phi(x)$ mapping $x$ to a new set of "features". Before we arrive at this new representation, let's state Mercer's Theorem:

<span style="color:#0060C6">**Theorem 3 (Mercer's Theorem):** Let $K$ be a symmetric, positive-definite function. Then, define the linear operator</span>
...
<center><span style="color:#0060C6">$$T_{K}(\phi)(\boldsymbol{x}) := \int_{a, b} K(\boldsymbol{x}, \boldsymbol{x}) \phi(\boldsymbol{s}) d\boldsymbol{s}$$
