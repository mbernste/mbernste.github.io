...
How a metric is used depends a lot on what the organization wants from these metrics. In some cases, an organization is seeking to approximate, or define, the value function $V$. That is, they choose a metric $f$ and this metric serves as a _proxy_ for $V$. This is thus, a more formal way to describe Bezos's description of how organizations use metrics as "proxies" for value.

In other cases, organizations will create a _collection_ of metrics, each describing some specific aspect of the system: $f_1, f_2, ..., f_M$. In this way, one can reduce a complex, high-dimensional system down into the space of a few dimensions:
...
The different ways an organization uses metrics bring us to the two high-level categories that metrics can fall into: exploratory metrics and value-measuring metrics. 

### Exploratory metrics: Those that seek to estimate $\mathcal{X}$

Metrics can often serve utility without any attempted to relation to the value function $V$. That is, as mentioned previously, a colleciton of metrics, $f_1(x), f_2(x), \dots, f_M(x)$, serve as a form of dimensionality reduction. That is, they help an organization _understand_ the system under study. 

### Value-measuring metrics: Those that seek to estimate $V$
