...
In this situation, the metric $f$ is _proportional_ to $V$ (with a possible shift). That is $V(x) = cf(x) + k$ for some unknown constants $c$ and $k$. This is illustrated in the figure below:

This is actually a very nice situtation to find oneself in; however, it comes with a key challenge: One cannot know when we they have achieved success. If we define some success criteria, or threshold, in terms of $V$, and $f$ is proportional to $V$, then we will not in principal know exactly when we have achieved success. Thus, the key challenge in this scenario, is knowing when one has succeeded.
