...
When one has such a metric $f$, then one can use it reliably to determine whether or not they are improving the system, $x$. That is, if one is considering two systems, $x_1$ and $x_2$, for which $f(x_1) > f(x_2)$, then one can be sure that $V(x_1) > V(x_2)$ and therefore one should prefer $x_1$. However, one _cannot_ rely on $f$ for either of the two use cases:

* One cannot use $f$ to determine _how much_ the system improved. That is, it may be that $f(x_1) > f(x_2)$, however, the difference $f(x_1) - f(x_2)$ may not closely track with $V(x_1) - V(x_2)$.
* One cannot use $f$ to reliably determine success criteria. That is because while $f$ tracks $V$ directionally, it does not tell you about the absolute magnitude of $V$.  
