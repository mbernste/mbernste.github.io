...
In this challenge, groups found that they could distort the normalization of the data such that differential expression overlap was maximized; however, the models were nonsensical. In fact, users could make completely random predictions and still optimize the objective function. Clearly, this metric, while perhaps a locally good proxy of the latent value function (in the regime for which the Arc Institute designed the challenge), but a poor proxy of latent value in distal regions of the design space. 


**Proportional to $V$** 

In this situation, the metric $f$ is _proportional_ to $V$. That is $V(x) = cf(x)$ for some unknown constant $c$. This is illustrated in the figure below:
