...
At their foundation, a VAE defines a probabilistic generative process for "generating" data points that reside in some $D$-dimensional vector space. This generative process goes as follows: we first sample a latent variable $\boldsymbol{z} \in \mathbb{R}^{J}$ where $J < D$ from some distribution, and then use a determinstic function to map $\boldsymbol{z}$ to $\boldsymbol{x}$. Typically, $\boldsymbol{z}$ is made to follow a standard normal distribution:
