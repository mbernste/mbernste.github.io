...
_Variational autoencoders (VAEs) are a family of deep generative models with use cases that span image processing to bioinformatics. There are two complimentary ways of viewing VAEs: as a probabilistic model that is fit using variational Bayesian inference or as a type of autoencoding neural network.  In this post, we present the mathematical theory behind VAEs, rooted in Bayesian inference, and how this theory leads to an emergent autoencoding algorithm. We also discuss the similarities and differences between VAEs and standard autoencoders. Lastly, we present an implementation of a VAE in PyTorch and apply it to modeling the MNIST dataset of hand-written digits._ 
