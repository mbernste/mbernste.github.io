...
<span style="color:#2284EB">**Definition 1**: A **probability space** is a measure space ($\Omega$, $E$, $P$) where $P(\Omega) = 1$ where 
1. The set $\Omega$, is called the **sample space**.
2. The sigma-algebra over $\Omega$, denoted $E$, called the set of **events**.
3. The measure $P$ for the measureable space $(\Omega, E)$ is the **probability measure**.</span>
