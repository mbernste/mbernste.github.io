...
A key point to note in the above matrix equation is that the dimensionality of the weights vector, $\boldsymbol{W}$, does not depend on the number of nodes in the graph. Thus, we see that the graph convolutional layer can operate on graphs of any size so long as the feature vectors at each node are of the same dimension!
...
The embedding of each node, $\boldsymbol{h}_i$, that is output by the graph convolutional layer can treated as input to the next layer! Thus, we can create a multi-layer GCN by stacking graph convolutional layers together. This would be described as:
...
One idea to mitigate this issue would be to take the _average_ of the neighbors vectors rather than the sum. That is to compute,

$$\bar{\boldsymbol{x}}_i = \frac{1}{\left\vert \text{Neigh}(i) \right\vert}\sum_{j \in \text{Neigh}(i)} \boldsymbol{x}_j$$

Here, for node $i$, we simply divide the sum by the number of neighbors of node $i$. We can perform this averaging across all nodes using the following matrix multiplication:
