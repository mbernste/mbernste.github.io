...
In computational biology, graph neural networks are commonly applied to computational tasks operating on moleculular structures. A graph is a natural data structure for encoding a molecule where each node represents an atom and each edge connects two atoms that are bonded together.
