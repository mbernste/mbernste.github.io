...
where $\boldsymbol{H}_1$, $\boldsymbol{H}_2$, and $\boldsymbol{H}_2$ are the embedded node vectors at layers 1, 2 and 3 respectively. The matrices $\boldsymbol{W}_1$, $\boldsymbol{W}_2$, and $\boldsymbol{W}_3$ are the weight matrices that parameterize each layer. A schematic illustration of stacked graph convolutional layers is depicted below:
...
As a case-study for applying GCNs to a real task, we will implement and train a GCN to classify molecular toxicity. This is a binary-classification task where we are provided a molecule and our goal is to classify it as either toxic or not toxic. More specifically, we will use the [Tox21]() dataset downloaded from [MoleculeNet](https://moleculenet.org/datasets-1). Applying GCNs to this dataset has been explored by the scientific community (e.g.,  [Chen _et al_. (2021)](https://doi.org/10.1186/s13321-021-00570-8)), but we will reproduce such efforts here. 

Using [PyTorch](https://pytorch.org/), we apply a GCN to the task of predicting XXXXX toxicity as encoded in the "NR-AhR" column of the dataset. The full notebook implementing this analysis can be found [on Google Colab](https://colab.research.google.com/drive/19efZG3SSbLQ9j0vxwAUkbU2o65xb8maL?usp=sharing), but we will present the key components here. 

In this dataset, each molecule is represented as a [SMILES](https://en.wikipedia.org/wiki/Simplified_molecular-input_line-entry_system) string. To decode these molecules into graphs, we use the [pysmiles](https://github.com/pckroon/pysmiles) Python package, which converts each string to a [NetworkX](https://networkx.org/) graph.

We then split the molecules/graphs into a random training set (85% of the data) and test set (the remaining 15%). For node features, we use 1) the element of the atom, 2) the number of implicit hydrogens, 3) the charge of the atom, and lastly, 4) the aromaticity of each atom.  

Next, we normalize each adjacency matrix using the following function:
...
The GCN model itself is implemented below:
...
Finally, the training loop is implemented as:
...
def train_gcn(A, X, y, batch_size=100, n_epochs=10, lr=0.1):
  ...
  ----------
  A
    A list of adjacency matrices corresponding to each graph in
    the training set.
  X
    A list of feature-matrices storing the node-specific features
    for each graph in the training set (in the same order as `A`).
  y
    The ground-truth binary label for each graph in the training
    set (in the same order as `A` and `X`.
  batch_size
    Size of each batch to be used in stochastic gradient descent.
  n_epochs
...
Applying the model to the test set, we achieve an AUROC of 0.87 and the following [ROC curve](https://en.wikipedia.org/wiki/Receiver_operating_characteristic) and [precision-recall curve](https://scikit-learn.org/stable/auto_examples/model_selection/plot_precision_recall.html#:~:text=The%20precision%2Drecall%20curve%20shows,a%20low%20false%20negative%20rate.):
...
The full code for the toxicity analysis is shown below (and can be run on [Google Colab](https://colab.research.google.com/drive/19efZG3SSbLQ9j0vxwAUkbU2o65xb8maL?usp=sharing)).
