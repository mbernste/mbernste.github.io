...
This procedure is often called **message passing** since each vector is "passing" its vector to its neighbors to update their vector. Each node's "message" is the vector that is associated with it.
