...
The matrix, $\boldsymbol{D}^{-1/2}(\boldsymbol{A}+\boldsymbol{I})\boldsymbol{D}^{-1/2}$ can then be interpreted as the process of "normalizing" the adjacency matrix by computing edge weights between adjacent nodes such that a weight will be smaller if the node has many neighbors and larger if it has few neighbors. We will discuss the intuition into this normalization in the next section, but for now, the important point to keep in mind is that element $i,j$ of the matrix $\boldsymbol{D}^{-1/2}(\boldsymbol{A}+\boldsymbol{I})\boldsymbol{D}^{-1/2}$ will have a non-zero entry only if there is an edge between nodes $i$ and $j$. We will discuss the inutuition behind this normalization in the next section. For now, we will simplify this term and let $\tilde{\boldsymbol{A}}$ denote the normalized adjacency matrix. That is,

$$\tilde{\boldsymbol{A}} := \boldsymbol{D}^{-1/2}(\boldsymbol{A}+\boldsymbol{I})\boldsymbol{D}^{-1/2}$$

Then, the graph convolutional layer function can be simplified to :
...
The matrix $\boldsymbol{D}$ is the [degree matrix](https://en.wikipedia.org/wiki/Degree_matrix) of $\boldsymbol{A}+\boldsymbol{I}$. This is a diagonal matrix where element $i,i$ stores the total number of neighboring nodes to node $i$ (including itself). Then, $\boldsymbol{D}^{-1/2}$ is simply the diagonal matrix formed by the reciprocal of the square root of each entry in $D$. That is, if we have
...
$$\boldsymbol{D}^{-1/2} := \begin{bmatrix}d_{1,1} & 0 & 0 & \dots, 0 \\ 0 & d_{2,2} & 0 & \dots & 0 \\ 0 & 0 & d_{3,3} \dots 0 \\ 
\vdots & \vdots & \vdots & \ddots & \vdots \\ 0 & 0 & 0 & \dots & d_{n,n}\end{bmatrix}$$

 Now, what is the interpretation of $\boldsymbol{D}^{-1/2}(\boldsymbol{A}+\boldsymbol{I})\boldsymbol{D}^{-1/2}$? We can view this as "normalized" adjacency matrix where, if we sum the values in each row of the matrix, we will get a value in the same relative range.
