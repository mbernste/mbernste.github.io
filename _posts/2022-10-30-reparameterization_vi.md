...
This may remind you of the [EM algorithm](https://mbernste.github.io/posts/em/), where we iteratively optimize the ELBO in terms of a surrogate distribution $q$ (which in VI is the variatonal posterior) and the model parameters $\theta$. Here we are doing the same sort of thing, but instead of using a coordiante ascent algorithm, we are using a gradient ascent algorithm.  One key difference to keep in mind, however, is that at the $t$th step of the coordinate ascent algorithm behind EM, $q$ is maximized by finding the _exact value_ for $p_{\theta_t}(z \mid x)$ where $\theta_t$ is the current estimate of $\theta$ at time step $t$. In constrast, in VI, the variatonal family $\mathcal{Q}$ may not include $p_{\theta_t}(z \mid x)$ and thus, the final point estimate of $\theta$ is not gauranteed to be a local maximum of the log-likelihood like it is in EM. In practice, however, maximizing the lower bound for the log-likelihood often works well even if it does not have the same gaurantees as EM.
