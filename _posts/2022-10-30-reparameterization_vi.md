...
$$ELBO(\boldsymbol{\beta}) := E_{\boldsymbol{\epsilon} \sim N(\boldsymbol{0}, \boldsymbol{I})}\left[\sum_{j=0}^J \log N(\beta_j; 0, 10)\sum_{i=1}^n \log N(y_i; (\boldsymbol{mu} + \boldsymbol{\epsilon} \odot \boldsymbol{\sigma})^T\boldsymbol{x}_i + \beta_0, \sigma^2) - \sum_{j=0}^J \log N(\mu_j + \epsilon_j \sigma_j; \mu_j, \sigma^2_j)\right]$$
