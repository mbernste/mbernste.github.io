...
It turns out that the real numbers themselves are themselves a vector space (when equipped with standard addition and multiplication) in which the real numbers are both vectors and scalars! This example may be a bit trivial and silly; however, I like it because it highlights the generality of the definition of a vector space. In the vector space consisting of real numbers, the number zero acts as the zero vector. 
...
Sets of functions themselves can form vector spaces! In fact, the real power in the definition for a vector space reveals itself when dealing with functions, and the fact that some sets of functions form vector spaces lies at the foundation for many fundamental ideas in mathematics, physics, and statistics including [Fourier transforms](https://en.wikipedia.org/wiki/Fourier_transform) and [reproducing kernel hilbert spaces](https://en.wikipedia.org/wiki/Reproducing_kernel_Hilbert_space).
