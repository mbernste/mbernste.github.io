...
$$\begin{align*}\frac{\partial L(\Theta, \lambda)}{\partial \boldsymbol{\mu}_k} := \sum_{i=1}^n \sum_{k=1}^K \gamma_{t,i,k} \frac{\partial \log \alpha_k \phi(\boldsymbol{x}_i ; \boldsymbol{\mu}_k, \boldsymbol{\mu}_k)}{\partial \boldsymbol{\mu}_k} \end{align*}$$
