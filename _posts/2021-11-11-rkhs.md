...
Now, one may ask what exactly is each $x \in \mathcal{X}$ being mapped to?  That is, what is each $\phi(x)$?  Let's say we have some positive definite function $K(x, y)$ and let's fix $x$ and let this be only a function of $y$. Then, we can see that

$$\begin{align*}K(x, y) &= \langle \phi(x), \phi(y) \rangle \\  &= \delta_{\phi(y)}(\phi(x)) \\ &= \phi(x)(y)\end{align*}$$

What does this mean? it means that the our $\phi(x)$ is simply the function $K(x, .)$! That is, it the univariate function formed by taking $K$ and fixing the first argument to $x$.
