...
where $\phi(x)$ is the vector associated with object $x$.

To use a kernel method in practice, one can get pretty far with only this understanding; however, if it is incomplete and to me, unsatisfying. To really undersatnding the intricases of kernel methods, one must understand a mathematical object called a **reproducing kernel Hilbert space** (RKHS).  

In this post, I will explain the definition of a RKHS and some of its properties in order to provide a foundation for better intuiting kernel methods, like support vector machines, beyond the introductory explanation often provided to new students.  
