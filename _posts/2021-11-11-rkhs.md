...
Choose a vector $\hat{f} \in \text{ker}(\delta_x)^{\perp}$ where $\text{ker}$ is the kernel (i.e., null space) of the $\delta_x$. Furthermore, let this be a unit vector. That is, $\vert\vert\hat{f}\vert\vert = 1$. Now, let's define a vector $k_x := \delta_x(\hat{f})\hat{f}. We'll now show that this is the function that can be used to reproduce $\delta_x$.
