...
<span style="color:#0060C6">**Definition 1 (Reproducing kernel Hilbert space):** Given a Hilbert space $(\mathcal{H}, \mathcal{F}, \langle ., . \rangle)$ where $\mathcal{H}$ is the set of vectors, $$\mathbb{F}$$ are the set of scalars, $\langle ., . \rangle$ is the inner product, and $f \in \mathcal{H}$ is a function $f : \mathbb{S} \rightarrow \mathbb{R}$, where $\mathcal{X}$ is some set, then  $(\mathcal{H}, \mathcal{F}, \langle ., . \rangle)$ is a **reproducing kernel Hilbert space** if given any $x \in \mathcal{X}$, the evaluation functional for $x$, $\delta_x(f) := f(x) (where $f \ in \mathcal{H})$, is continuous.</span>
