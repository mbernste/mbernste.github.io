...
What if we choose a different evaluation functional for another value $y \in \mathcal{S}$? Then, there exists a different function $k_y$ that we can use to reproduce $\delta_y$.  Now, recall the function $k_x$ (used to reproduce $\delta_x$) is an element of $\mathcal{H}$ like any other. We thus see that

$$\delta_y(k_x) = <k_x, k_y>$$

If you notice, we have taken two arbitrary values, $x, y \in \mathcal{S}$ and mapped them to two vectors $$k_x, k_y \in \mathcal{H}$$ and then computed their inner product. This is the very notion of a kernel that we discussed in the introduction to this blog post!

That is, we can construct a function $K$ that operates on pairs of elements in $x, y \in \mathcal{S}$:

$$K(x, y) := \delta_y(k_x) = <k_x, k_y>$$

This is the **reproducing kernel** (or simply **kernel**) of the RKHS! To make this clearer, we can denote $$\phi : \mathcal{S} \rightarrow \mathcal{H}$$ to be the function that maps elements in $\mathcal{S}$ to their corresponding element in the Hilbert space that reproduces its corresponding evaluation functional! That is, 
