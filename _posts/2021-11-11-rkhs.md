...
<span style="color:#0060C6">**Definition 1 (Reproducing kernel Hilbert space):** Given a Hilbert space $(\mathcal{H}, \mathcal{R}, \langle ., . \rangle)$ where $\mathcal{H}$ is the set of vectors, $$\mathbb{R}$$ are the real numbers and set of scalars for the vector space, and each vector $f \in \mathcal{H}$ is a function $f : \mathbb{R} \rightarrow \mathbb{R}$, $(\mathcal{H}, \mathcal{R}, \langle ., . \rangle)$ is a **reproducing kernel Hilbert space** if given any $x \in \mathbb{R}$, the evaluation functional for $x$, $\delta_x(f) := f(x)$ is continuous.</span>
