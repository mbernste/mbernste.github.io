...
If you're a practitioner of machine learning, then there is little doubt you have heard of, and likely used, an algorithm that falls into the general category of **kernel methods**. The premier example of a kernel method is the [support vector machine](https://en.wikipedia.org/wiki/Support-vector_machine). When introduced to these algorithms, one is taught that, owing to the [kernel trick](https://en.wikipedia.org/wiki/Kernel_method), one must provide the algoirthm with a **kernel function** that encodes some measure of pairwise "similarity" between your objects. 
...
To use a kernel method in practice, one can get pretty far without having a deep understanding into what these kernels actually are. This is owing to the fact that a number of popular kernels, such as the radial-basis function kernel or polynomial kernel, are implemented in common machine learning libraries like [scikit-learn](https://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html#sklearn.svm.SVC).  
