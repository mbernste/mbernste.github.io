...
This marginalization requires solving an integral over all of the parameters of a neural network! This is not feasible to calculate. Similarly, it is difficult to estimate the values for $\theta$ by maximizing the data likelihood due to the need to marginalize over each $\boldsymbol{z}\_i$:
