...
def loss_function(output, x, mu, logvar):
    ...
    return recon_loss + (0.5 / batch_size)  * kl_loss
...
Another quick point to note about the above implementation is that we weighted the KL-divergence term in the loss function by `(0.5 / batch_size)`. This weighting can be interpreted as hard-coding a larger variance in the normal distribution $p\_\theta(\boldsymbol{x}\_i \mid \boldsymbol{z}\_i)$.
