...
* We can represent a graph as a matrix by encoding which nodes are connected with eachother. That is, if we have a graph with $$n$$ nodes, then we can define a matrix $$\boldsymbol{X}$$ such that $$x_{i,j}$$ is 1 if node $$i$$ is connected to node $$j$$ and zero otherwise. This is called an [adjacency matrix](https://en.wikipedia.org/wiki/Adjacency_matrix) for a graph. For example, the following graph

can be represented as the following adjacency matrix:
