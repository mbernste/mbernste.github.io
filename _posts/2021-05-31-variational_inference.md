...
This is precisely the problem addressed in the calculus of variations.  In the case of variational inference, the functional, $g$, that we are optimzing is the ELBO.  The set of functions, $\mathcal{F}$, that we are searching over are the set of [probability measures](https://mbernste.github.io/posts/measure_theory_1/) in the variational family, $\mathcal{Q}$.
