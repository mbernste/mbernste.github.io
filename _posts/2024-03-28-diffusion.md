...
where $\text{max}, \text{min} \in [0,1]$ and $\text{min} < \text{max}$ are two small constants. The function above will compute a sequence of $\beta_1, \dots, \beta_T$ that interpolate linearly between $\text{min}$ and $\text{max}$.

This begs the question: Why use a different value of $\beta_t$ at each time step? Why not set $\beta_t$ constant across timesteps? Empirically, if $\beta_t$ is large, then the object will turn to noise too quickly washing away signal too early in the process making it challenging to learn $p\_\theta(\boldsymbol{x}_{t-1} \mid \boldsymbol{x}_t)$. In contrast, if $\beta$ is very small, then each step only removes a very small amount of noise, and thus, to turn an object $\boldsymbol{x}_0$ into white noise (and back via reverse diffusion) would require many timesteps (which as we will see, would lead to inefficient training of the model). A solution is thus to increase the variance gradually so that at the beginning of the diffusion process, only a little bit of noise is added a time, but towards the end of the process we add more noise to ensure that $\boldsymbol{x}_T$ is almost pure noise. This is illustrated in the figure below:
...
### Derivation 3 (Scaling the mean by $\sqrt{1-\beta}$ constrains the variance):
