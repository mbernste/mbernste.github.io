...
A solution that balances the need to maintain the object's structure, but keep the number of timesteps relatively short is to increase the variance at each timestep according to a set schedule so that at the beginning of the diffusion process, only a little bit of noise is added a time, but towards the end of the process we add more noise at a time to ensure that $\boldsymbol{x}_T$ approaches a sample from $N(\boldsymbol{0}, \boldsymbol{I})$ (i.e., it becomes pure noise). This is illustrated in the figure below:
