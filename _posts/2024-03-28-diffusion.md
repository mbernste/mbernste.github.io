...
$$\begin{align*}E_{\boldsymbol{x}_{1:T} \left[\frac{p_\theta(\boldsymbol{x}_T)}{q(\boldsymbol{x}_T \mid \boldsymbol{x}_0)} \right] &= \int \int q(\boldsymbol{x}_T) \log \frac{p_\theta(\boldsymbol{x}_T)}{q(\boldsymbol{x}_T \mid \boldsymbol{x}_0)} \ d\boldsymbol{x}_T \end{align*}$$
