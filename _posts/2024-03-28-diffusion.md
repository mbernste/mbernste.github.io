...
This begs the question: Why use a different value of $\beta_t$ at each time step? Why not set $\beta_t$ constant across timesteps? Empirically, if $\beta_t$ is large, then each step of the diffusion process adds quite a lot of noise to the object at each step. Each reverse diffusion posterior, $p\_\theta(\boldsymbol{x}_{t-1} \mid \boldsymbol{x}_t)$, would then need to know how to remove a lot of noise and make the image a lot clearer. This is a harder learning task than learning how to remove a small amount of noise to make the object just slightly clearer. In contrast, if $\beta$ is very small, then each step only removes a very small amount of noise, and thus, to turn an object $\boldsymbol{x}_0$ into white noise (and back via reverse diffusion) would require many timesteps (which as we will see, would lead to inefficient training of the model). A nice balance is to increase the variance gradually so that at the beginning of the diffusion process, only a little bit of noise is added a time to XXXXXX. This is illustrated in the figure below:
