...
The second line follow from a reparameterization of $\boldsymbol{x}_t$ by noting the previously described closed form or $q(\boldsymbol{x}_t \mid \boldsymbol{x}_0)$ and seeing that $\boldsymbol{x}_t$ can be sampled by first sample $\epsilon_t \sim N(\boldsymbol{0}, \boldsymbol{I})$ and then generated by the function,
...
$$\begin{align*}\tilde{\boldsymbol{\mu}}(\boldsymbol{x}_{t}) &:= \frac{1}{\sqrt{\alpha_t}} \left(\boldsymbol{x}_t - \frac{\beta_t}{\sqrt{1- \bar{\alpha}_{t}}}\epsilon_t \right) \\ \tilde{\sigma}_t^2 &:= \frac{\beta_t \left(1 - \bar{\alpha}_{1-t}\right)}{1- \bar{\alpha}_t}\boldsymbol{I}\end{align*}$$
...
$$L_t := E_{\boldsymbol{x}_t \sim q} \left[ \frac{1}{2\sigma_t^2} \vert\vert\boldsymbol{\mu}_\theta(\boldsymbol{x}_t, t) - \tilde{\boldsymbol{\boldsymbol{\mu}}}(\boldsymbol{x}_t, t)\vert\vert^2 \right]$$

Here we see that to optimize the objective function, we simply must minimize the mean squared error between the mean of $q(\boldsymbol{x}_{t-1} \mid \boldsymbol{x}_t, \boldsymbol{x}_0)$, given by $\tilde{\boldsymbol{\mu}}(\boldsymbol{x}_t, t)$, and the mean of $p_\theta(\boldsymbol{x}_{t-1} \mid \boldsymbol{x}_t). While this is a nice result and could be used for training, [Ho, Jain, and Abbeel (2020)](https://arxiv.org/pdf/2006.11239.pdf) found that a further modification to the objective function improved the stability of training and performance of the model.

Specifically, the authors proposed reparameterizing the function $\boldsymbol{\mu}(\boldsymbol{x}_t, t)$ (i.e., the trainable model) as follows:

$$\boldsymbol{\mu}(\boldsymbol{x}_t, t) := \frac{1}{\sqrt{\alpha_t}}\left( \boldsymbol{x}_t - \frac{\beta_t}{\sqrt{1-\bar{\alpha}_t}}\epsilon_\theta(\boldsymbol{x}_t, t) \right)$$
...
\boldsymbol{x}_t(\boldsymbol{x}_0, \epsilon) + \frac{\sqrt{\bar{\alpha}_{t-1}}\beta_t}{1-\bar{\alpha}_t} \boldsymbol{x}_0 \\ &= \frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1})}{1-\bar{\alpha}_t}
