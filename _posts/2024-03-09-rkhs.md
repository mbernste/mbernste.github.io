...
Note, $\mathcal{H}$ does not actually form a plane (it is not $\mathbb{R}^2$), but this image of a plane emphasizes that the functions in $\mathcal{H}$ are infinitely dense and that two similar functions in $\mathcal{H}$ will be mapped to two similar values of $\delta_x$. (If you can think of a better schematic emphasizing these points, please let me know!).
