...
In fact, all of the [axioms of a vector space](https://mbernste.github.io/posts/vector_spaces/) can be shown to hold for certain sets of functions. Moreover, one can even form a Hilbert space of functions so long as the functions are "infinitely dense" and one can device an inner-product between functions. As its name suggests, a RKHS is a Hilbert space of functions.
