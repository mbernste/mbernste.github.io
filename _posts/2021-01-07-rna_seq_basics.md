...
$$\begin{align*} \theta_i &= \frac{t_i}{\sum_{j=1}^G t_j} \\ &= \frac{ \frac{n_i}{l_i} }{ \sum_{j=1}^G \frac{n_j}{l_j}} && \text{because} \ n_i = l_it_i \implies t_i = \frac{l_i}{n_i} \\ &= \frac{\frac{p_i}{l_i \sum_{j=1}^G n_j}}{ \sum_{j=1}^G \frac{p_j}{l_j \sum_{k=1}^G n_k}} \end{align*}$$
